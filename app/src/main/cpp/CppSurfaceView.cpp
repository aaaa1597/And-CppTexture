...
GLuint CppSurfaceView::createProgram(const char *vertexshader, const char *fragmentshader) {
    ...
    if(linkStatus != GL_TRUE) {
        ...
        if(bufLen) {
            ...
            glGetProgramInfoLog(mProgramId, bufLen, NULL, logstr);
...
void CppSurfaceView::predrawGL() {
    /* 各バッファ設定 */
    ma_Position = glGetAttribLocation(mProgramId, "a_Position");
    glEnableVertexAttribArray(ma_Position);
    ma_Texcoord = glGetAttribLocation(mProgramId, "a_Texcoord");
    glEnableVertexAttribArray(ma_Texcoord);
    ...
    /* サンプラ取得 */
    mu_Sampler = glGetUniformLocation(mProgramId, "u_Sampler");
...
void CppSurfaceView::drawGL() {
    ...
    /* テクスチャIDとテクスチャサンプラーとテクスチャUnitNoを関連付け */
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, mTextureId);
    glUniform1i(mu_Sampler, GL_TEXTURE0-GL_TEXTURE0);

    /* 座標点 */
    glVertexAttribPointer(ma_Texcoord, 2, GL_FLOAT, GL_FALSE, 0, texcoords);
    glVertexAttribPointer(ma_Position, 3, GL_FLOAT, GL_FALSE, 0, vertices);
